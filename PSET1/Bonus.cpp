...
// control convolution benchmark behavior
static bool conv_benchmark_empty_cache = true;

// a helper function which is used to determine if fast depthwise FP16 cuDNN conv kernels is required for workload
template <typename T>
...
// check if depthwise convolution should be used for cudnn 8.2 and above
template <typename T>
...
// check if xnnpack should be used, for mobile specific uses
...
// (this comment is from the github repo, not replacing as I would have to closely follow it for explaining) This struct is templated so that we can run backend selection in a dynamic shapes context; all of the real kernel selection in eager mode runs with int64_t
// main struct that initializes all the convolution parameters and helper functions
template <typename T>
struct ConvParams {
    // all convolution parameters
    std::vector<T> stride;
    ...
    // all of the below bool functions determine which backend should be used for the convolution
    ...
    bool use_cudnn_depthwise(const at::Tensor& input, const at::Tensor& weight) const  {
        ...
        if (cudnn_version >= 8200) {
            bool kernel_cond =  (use_cudnn(input, weight) &&
                                ...
                                input.ndimension() == 4 &&
                                !is_dilated() &&
                                (stride[0] == stride[1] || at::symint::size<T>(input, 2) == 1) &&
                                at::symint::size<T>(input, 1) >= 32);
            if (kernel_cond) {
        ...
        bool kernel_cond =  (cudnn_version >= 7600 &&
                            ...
                            is_depthwise(input, weight) &&
                            input.ndimension() == 4 &&
                            at::symint::size<T>(weight, 2) == at::symint::size<T>(weight, 3) &&
                            at::symint::size<T>(input, 2) >= 7 &&
                            !is_dilated() &&
                            stride[0] == stride[1] &&
...
// for different convolution operations, define the dispatchers
...
// check the input shapes and sizes for various convolutions
template <typename T>
...
template <typename T>
// check shape of inputs for the backward pass
...
// (these set of comments are from the github repo, not replacing as I would have to closely follow it for explaining)
...
at::Tensor complex_convolution(
  ...
  // (these set of comment are from the github repo, not replacing as I would have to closely follow it for explaining)
...
at::Tensor complex_convolution_mode(
  ...
  auto i = c10::Scalar(c10::complex<double>(0, 1));
  // if these tensors are using GPU memory then the following tensor operations are accelerated
  return a - b + i * (c - a - b);
}
...
at::Tensor conv1d_symint(
  ...
  if (at::isComplexType(input_.scalar_type())) {
    ...
    // the following convolution is accelerated if using a GPU
...
at::Tensor conv2d_symint(
  ...
  if (at::isComplexType(input_.scalar_type())) {
    ...
    // following convolution is accelerated if using a GPU
...
// convolution implementation with same padding mode
...
// convolution implementation with different padding modes
...
// 1d convolution implementation with padding type as a passed parameter
...
// 2d convolution implementation with padding type as a passed parameter
...
// 3d convolution implementation with padding type as a passed parameter
...
// 1d transposed convolution implementation(?)
...
// main convolution function implemented by calling _convolution(define below)
...
template <typename T>
// select the correct convolution backend by calling this
...
// select a convolution backend with relevant input and parameters by calling _select_conv_backend
...
// implement convolutions by calling this function, this function handles the different backends and parameters that are passed
at::Tensor _convolution(
  ...
  // select the correct backend to use
  ...
  auto kernel_size = weight.sizes().slice(2);
  switch (backend) {
    // call GPU based depthwise 2d convolution(Cuda)
    case ConvBackend::CudaDepthwise2d:
      ...
      break;
    // call GPU based depthwise 3d convolution(Cuda)
    ...
    case ConvBackend::Cudnn:
      check_input_same_type_as_parameters(input, weight, bias);
      // call GPU based convolution(Cudnn)
      ...
      break;
    // call GPU based transposed convolution(Cudnn)
    case ConvBackend::CudnnTranspose:
      check_input_same_type_as_parameters(input, weight, bias);
...
// Computes gradients for input, weight, and bias depending on the output_mask setting. 
// This function supports 1D, 2D, or 3D spatial convolution and currently requires a single batch dimension to be present.
//
// Args:
...
std::tuple<Tensor, Tensor, Tensor> convolution_backward(
    ...
    // initialize the tensors and parameters
    auto grad_output = grad_output_;
    auto input = input_;
    auto weight = weight_;

    // get the number of dimensions for the filter
    auto k = weight.ndimension();
    int64_t dim = k - 2;
  ...
  // convert 2D inputs back to 1D for backends that don't natively support 1D spatial inputs.
  if (output_mask[0]) {
    if (k == 3 && !input.is_mkldnn() && !input.is_xpu()) {
